    ...
    def handle(self, *args, **options):
                ...
                for c in u['cards']:
                    ...
                    if c not in [x.card_id for x in ecs]:
                        try:
                            ...
                            self.stdout.write(e)
                            self.stdout.write(
                                "error adding new card for %s, card too long? %s" % (eu, c))
                eu.save()
            else:
                with transaction.atomic():
                    nu = User(id=int(u['id']), name=u['nick'],
                    ...
                    for c in u['cards']:
                        try:
                            ...
                            if not u['subscribed']:
                                self.stdout.write(
                                    "skipping card %s, it's already in the db and this user is not subscribed" % (c,))
                                continue
                            else:
                                # darn, maybe the other user is not subscribed?
                                if not ec.user.subscribed:
                                    self.stdout.write(
                                        "card %s is already in the db for %s, but they are not subscribed so i'm deleting the card" % (c, ec.user))
                                    ec.delete()
                                else:
                                    self.stdout.write(
                                        "panic: card id %s is in use by 2 users: %s and %s" % (c, ec.user, nu))
                        except ObjectDoesNotExist:
                            pass
                        except Exception as e:
                            self.stdout.write(u)
        ...
        for au in User.objects.all():
            if au.id not in carddb_by_id:
                self.stdout.write(
                    "user with id %d is in the local db, but not in carddb.json, unsubscribeing them and removing their cards" % (au.id))
                # remove  cards
                cs = au.card_set.all()
                for c in cs:
                    c.delete()
