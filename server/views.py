...
def granttocard(request, tool_id, to_cardid, by_cardid):
    ...
    try:
        ...
        if bcp != 'maintainer':
            result = {'numeric_status': 0,
                      'error': 'By card is not a maintainer'}
            return makeResponse(request, result)
    except ObjectDoesNotExist as e:
        result = {'numeric_status': 0,
                  'error': 'By card does not have permissions'}
        return makeResponse(request, result)
...
def settoolusetime(request, tool_id, card_id, duration):
    ...
    l = Log(tool=t, user=c.user, message="Tool used for %d seconds" %
            (int(duration),), time=int(duration))
    l.save()
...
def get_tool_runtime(request, tool_id, start_time):
    ...
    start = datetime.datetime.fromtimestamp(
        float(start_time), timezone("Europe/London"))

    # filter out runs that lasted longer than 24 hours, see issue #5
    seconds = Log.objects.filter(tool=t).filter(date__gt=start).filter(
        message='Time Used').filter(time__lt=3600 * 24).aggregate(Sum('time'))['time__sum']
    ...
    #
    verbose = printable + \
        ' of lasing have occurred. Approximately %d hours until the tube dies.' % (
            1985 - hours)

    ret = []
    # {'name': name, 'seconds': secs, 'printable': 'HH:MM:SS', 'verbose': 'blah blah'}
    ret.append({'name': t.name, 'seconds': seconds,
               'printable': printable, 'verbose': verbose})
...
def calheatmap1(request):
    ...
    context = {"yearthen": yearthen, "monththen": monththen, "daythen": daythen,
               "loltest": 23456, "testjsonnumber": 1472100000, "logtimes": clogs, "loglen": clens}
