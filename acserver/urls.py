...
urlpatterns = [
    ...
    # /1/card/12345678
    url(r'^(?P<tool_id>\d+)/card/(?P<card_id>[a-fA-F0-9]+)$',
        server.views.card),
    # /1/maintainers
    url(r'^(?P<tool_id>\d+)/maintainers$', server.views.maintainerlist),
    # /1/grant-to-card/12345678/by-card/00112233445566
    url(
        r'^(?P<tool_id>\d+)/grant-to-card/(?P<to_cardid>[a-fA-F0-9]+)/by-card/(?P<by_cardid>[a-fA-F0-9]+)$', server.views.granttocard),
    # /1/status/0/by/33333333
    url(r'^(?P<tool_id>\d+)/status/(?P<status>[01])/by/(?P<card_id>[a-fA-F0-9]+)$',
        server.views.settoolstatus),
    ...
    url(r'^(?P<tool_id>\d+)/tooluse/time/for/(?P<card_id>[a-fA-F0-9]+)/(?P<duration>[0-9]+)$',
        server.views.settoolusetime),
    # /api/get_tools_summary_for_user/1
    url(r'^api/get_tools_summary_for_user/(?P<user_id>[0-9]+)$',
        server.views.get_tools_summary_for_user),
    # /api/get_tools_status
    url(r'^api/get_tools_status$', server.views.get_tools_status),
    # /api/get_tool_runtime
    url(r'^api/get_tool_runtime_since/(?P<tool_id>[0-9]+)/(?P<start_time>[0-9]+)$',
        server.views.get_tool_runtime),
